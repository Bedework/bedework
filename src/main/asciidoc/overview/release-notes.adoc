...
*This fix is needed for attachments to work correctly.* It probably does not affect public events as attachmentsare not (much?) used.
...
Additionally, ensure the OpenSearch schema is up to date
...
If you deploy your own copy of bedework wars and ears theer are changes that might affect you. Most of the properties which were chanegd by the bedework deployment process have been replaced with run-time wildfly properties or by values.
...
Note that this has led to a number of changes to the bw script. In particular to build and deploy the calendar execute
----
./bw bwcal
----
...
*Updated to wildfly 25.0.1.FINAL* appears to have better memory handling.
