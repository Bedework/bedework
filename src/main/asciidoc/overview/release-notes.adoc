...
*Core* RDATE only recurring events were not being inxexed correctly - instances didn't show up.

*WebClient* Filter out Inbox from result
*WebClient* Events were being moved incorrectly (from Inbox) causing indexing issues.
*WebClient* Fixed the timeview - events near the enf=d of the day appeared in the next days cell.
...
*Updated to wildfly 22* appears to have better memory handling.
